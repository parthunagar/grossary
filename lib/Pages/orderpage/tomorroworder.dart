...
class _TomorrowOrderState extends State<TomorrowOrder> {
  ...
  void getOrderList() async {
    ...
    print('getOrderList => storenextdayOrdersUri : $storenextdayOrdersUri store_id  $id');
  ...
  CircleAvatar buildStatusIcon(IconData icon, {bool disabled = false}) =>
    CircleAvatar(
      ...
      child: Icon(icon, size: 20, color: !disabled
  ...
  Container buildOrderInfoRow(BuildContext context, String price, String prodID, String orderStatus, {double borderRadius = 8}) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(borderRadius: BorderRadius.vertical(bottom: Radius.circular(borderRadius)), color: kWhiteColor),
      child: Row(
  ...
  GestureDetector buildPrintRow(BuildContext context, AppLocalizations locale, {double borderRadius = 8, dynamic cartId}) {
    var locale = AppLocalizations.of(context);
    return GestureDetector(
      onTap: (){
  ...
  GestureDetector buildPrintInvoice(BuildContext context, AppLocalizations locale, {double borderRadius = 8, dynamic cartId, TodayOrderMain orderMain}) {
    var locale = AppLocalizations.of(context);
    return GestureDetector(
      onTap: (){
        Navigator.of(context).pushNamed(PageRoutes.invoice, arguments: {'inv_details': orderMain}).then((value) {
            print('Navigator.of(context).pushNamed(PageRoutes.invoice)) => value : $value');
        }).catchError((e) {
          print('Navigator.of(context).pushNamed(PageRoutes.invoice)) => ERROR : $e');
        });
      },
      child: Container(
        height: 30,
  ...
  GestureDetector buildSearchRow(BuildContext context, AppLocalizations locale, {double borderRadius = 8, dynamic cartId}) {
    var locale = AppLocalizations.of(context);
    return GestureDetector(
      onTap: (){
        _startScanDevices();
                  ...
                  children: [
                    Row(
                      children: [
                        ...
                        SizedBox(width: 5),
                        Text('- '+mainP.user_phone, overflow: TextOverflow.ellipsis, style: Theme.of(context).textTheme.subtitle1.copyWith(color: kTextBlack,fontSize: 16,fontWeight: FontWeight.w700),
                        ),
                      ],
                    ),
                    SizedBox(height: 6),
          ...
          Row(
            children: [
              Container(
                ...
                child: Text(locale.orderID + ' #${mainP.cart_id}', textAlign: TextAlign.right, style: Theme.of(context).textTheme.subtitle2.copyWith(fontSize: 12,color: kOrderIdColor,fontWeight: FontWeight.w700)),
              ),
              SizedBox(width: 10),
  ...
  Column buildGreyColumn(BuildContext context, String text1, String text2, {Color text2Color = Colors.black}) {
    return Column(
      ...
      children: [
        ...
        SizedBox(height: 8),
        LimitedBox(maxWidth: 100, child: Text(text2, overflow: TextOverflow.ellipsis, style: TextStyle(fontWeight: FontWeight.w700, fontSize: 12, color: text2Color))),
      ],
    );
  }
