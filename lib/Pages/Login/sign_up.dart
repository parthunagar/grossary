...
class _SignUpState extends State<SignUp> {
  ...
  void hitCityData() {
    ...
    http.get(cityUri).then((value) {
      ...
      print('hitCityData ERROR : ${e.toString()}');
      setState(() {
        selectCity = 'Select your city';
        cityData = null;
        showDialogBox = false;
  ...
  _imgFromGallery() async {
    ...
    }).catchError((e) => print('_imgFromGallery ERROR : ${e.toString()}'));
  }
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: Row(
                          ...
                          children: [
                            Image(image: (_image != null) ? FileImage(_image) : AssetImage('assets/icon.png'), height: 80, width: 80),
                            SizedBox(width: 20.0),
                            Text(locale.uploadpictext, style: Theme.of(context).textTheme.headline6.copyWith(color: kMainTextColor, fontWeight: FontWeight.bold, fontSize: 18),),
                          ],
                        ),
                      ),
                    ),
                  ...
                  Padding(
                    ...
                    child: Text(locale.selectycity1, style: Theme.of(context).textTheme.headline6.copyWith(color: kMainTextColor, fontWeight: FontWeight.bold, fontSize: 21.7)),
                  ),
                  Container(
                    ...
                    child: DropdownButton<CityDataBean>(
                      ...
                      onChanged: (value) {
                        ...
                        print('selectCity : ${value.toString()}');
                      },
                    ),
                  ),
                  EntryField(
  ...
  void hitSignUpUrl(
    ...
    if (_image != null) {
      ...
      if(fid!=null && fid.length>0){
        http.MultipartFile.fromPath('store_doc', _image.path, filename: fid).then((pic) {
          requestMulti.files.add(pic);
          requestMulti.send().then((values) {
            values.stream.toBytes().then((value) {
              var responseString = String.fromCharCodes(value);
              var jsonData = jsonDecode(responseString);
              print('hitSignUpUrl jsonData : ${jsonData.toString()}');
              ...
              print('hitSignUpUrl ERROR 1 : ${e.toString()}');
              setState(() {    showDialogBox = false;   });
            });
          }).catchError((e) {
            print('hitSignUpUrl ERROR 2 : ${e.toString()}');
            setState(() {   showDialogBox = false;  });
          });
        }).catchError((e) {
          setState(() {  showDialogBox = false;  });
          print('hitSignUpUrl ERROR 3 : ${e.toString()}');
        });
      }
      else{
        print('not null');
        ...
        requestMulti.send().then((value1){
          value1.stream.toBytes().then((value) {
            ...
            print('hitSignUpUrl ERROR 4 : ${e.toString()}');
            setState(() { showDialogBox = false;  });
          });
        }).catchError((e){
          print('hitSignUpUrl ERROR 5 : ${e.toString()}');
          setState(() {  showDialogBox = false;  });
        });
      }
    }
      ...
      requestMulti.send().then((value1){
        value1.stream.toBytes().then((value) {
          ...
          print('hitSignUpUrl ERROR 6 : ${e.toString()}');
          setState(() {   showDialogBox = false;  });
        });
      }).catchError((e){
        print('hitSignUpUrl ERROR 7 : ${e.toString()}');
        setState(() {  showDialogBox = false;  });
      });
    }
  }
