...
class RechargeWalletState extends State<RechargeWallet> {
  ...
  void getPaymentList() async {
    ...
    http.get(paymentGatewaysUri).then((value) {
      print('getPaymentList recharge wallet => ppy : ${value.body}');
      ...
      print('getPaymentList recharge wallet => ERROR : ${e.toString()}');
      setState(() {
        isLoading = false;
      });
    });
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          ...
          style: TextStyle(color: kMainHomeText),
        ),
        centerTitle: true,
      ),
      body: Container(
                          ...
                          children: [
                            ...
                            Visibility(
                              ...
                              child: buildPaymentType(
                                ...
                                if (!isLoading) {
                                  ...
                                  //     "${paystackBean.paystack_public_key}",
                                  //     '$apCurrency',
                                  //     (int.parse('${rechargeController.text}') *
                                  //             100)
                                  //         .toInt());
                                }
                              }),
                            ...
                            Visibility(
                              ...
                              child: buildPaymentType(
                               ...
                               if(rechargeController.text!=null&&rechargeController.text!=''){
                                 try{
                                   double amount = double.parse('${rechargeController.text}');
                                   ...
                                   if(amount>0){
                                     ...
                                     Toast.show(locale.enterValidAmount, context,
  ...
  void checkOut(String amount, String paymentStatus, String paymentid,
      String paymentGateway) async {
    var locale = AppLocalizations.of(context);
    SharedPreferences prefs = await SharedPreferences.getInstance();
    var http = Client();
    print('$rechargeWalletUri ' +
        'user_id : ' +
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    if (response.paymentId != null) {
      ...
      checkOut('${rechargeController.text}', 'success', '${response.paymentId}',
          'razorpay');
    } else {
      setState(() {
        isLoading = false;
  ...
  void _handlePaymentError(PaymentFailureResponse response) {
    print('PaymentFailureResponse  : '  + response.message.toString());
    var jsData = jsonDecode(response.message);
  ...
  //       checkOut('${rechargeController.text}', 'success', '${value.reference}',
  //           'paystack');
  //     } else {
  ...
  void setStripePayment(dynamic clientScretKey, double amount,
      CreditCard creditCardPay, String apCurrency) {
    ...
    StripePayment.createPaymentMethod(PaymentMethodRequest(card: creditCardPay))
        .then((value) {
      createPaymentIntent('${newAmount.toInt()}', '$apCurrency', headers, value,
          clientScretKey);
    }).catchError((e) {
      ...
      http.post(paymentApiUrl, body: body, headers: hearder).then((value) {
        ...
        StripePayment.confirmPaymentIntent(
          ...
          if ('${paymentIntent.status}'.toUpperCase() ==
            ...
            checkOut('${rechargeController.text}', 'success',
      ...
      http
        ...
        if (value.body.toString().contains('error')) {
          ...
          if ('${chargeResp.status}'.toUpperCase() ==
              'succeeded'.toUpperCase()) {
            checkOut('${rechargeController.text}', 'success',
                '${chargeResp.paymentMethod}', 'stripe');
          } else {
            setState(() {
              isLoading = false;
            });
