...
class CancelPageState extends State<CancelPage> {
  ...
  void getHistoryList() async {
    ...
    http.get(cancellingReasonsUri).then((value) {
      print('getHistoryList cancel reason => ppy : ${value.body}');
      ...
      print('getHistoryList cancel reason ERROR : ${e.toString()}');
      setState(() {
        isLoading = false;
      });
    });
      ...
      body: Column(
        children: [
          AppBar(
            ...
            title: Text(locale.cancelreason, style: TextStyle(color: kRoundButtonInButton)),
            centerTitle: true,
          ),
          RowHistory(locale),
          Expanded(
                      ...
                      return GestureDetector(
                        onTap: () {
                          if (!isDelete) {
                            ...
                            hitDelete('${rechargeHistory[index].reason}', context);
                          }
                        },
                        ...
                        child: Card(
                          ...
                          child: Container(
                            ...
                            child: Row(
                              children: [
                                Text('${index + 1}', style: Theme.of(context).textTheme.headline1.copyWith(fontSize: 16),),
                                Expanded(
                                  child: Padding(
                                  padding: const EdgeInsets.all(8.0),
                                  child: Text('${rechargeHistory[index].reason}', style: Theme.of(context).textTheme.headline1.copyWith(fontSize: 16),
                                  ),
                                )),
                              ],
                            ),
  ...
  void hitDelete(dynamic reason, BuildContext context) async {
    print('hitDelete => deleteOrderUri : $deleteOrderUri  ' +
    ...
    http.post(deleteOrderUri, body: {
      ...
      print('hitDelete value.body :  ${value.body}');
      if (value.statusCode == 200) {
      ...
      print('hitDelete ERROR :  ${e.toString()}');
      setState(() {
        isDelete = false;
      });
    });
